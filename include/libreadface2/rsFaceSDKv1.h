 ...
 * license manager v2
 */
RS_API int CALLSPEC rsInitLicenseManagerV2(RShandle *handle, const char *licenseContent);
RS_API int CALLSPEC rsRunLicenseManagerV2AuthRequest(RShandle handle, char **authContent);
RS_API int CALLSPEC rsActivateLicenseManagerV2(RShandle handle, const char *authContent);
RS_API void CALLSPEC rsUnInitLicenseManagerV2(RShandle *handle);
...
RS_API int CALLSPEC rsRunFaceDetect(RShandle handle, const unsigned char *image,
    ...
    int *pFaceCount);
// face align
RS_API int CALLSPEC rsRunFaceAlign(RShandle handle, const unsigned char *image,
    rs_pixel_format pix_format,
    int width, int height, int stride, rs_image_rotation img_rotate,
    rs_rect rect,
    rs_face_info *pFace);
